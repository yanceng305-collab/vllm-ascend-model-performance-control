# Result: GLM-5.2-W8A8 16K Baseline (Evidence-Backed)

**Result ID**: `RESULT-GLM52-W8A8-16K-BASELINE-EVIDENCE-run-20260902-140958`  
**Model**: GLM-5.2-W8A8  
**Workload**: 16384 input tokens, 1024 output tokens  
...
- **Run 2**: 956.91 tok/s (completed: 256, failed: 0)
- **Run 3**: 959.19 tok/s (completed: 256, failed: 0)
- **Run 4**: 957.71 tok/s (completed: 256, failed: 0)

### Independent Calculation

**Mean (Run2, Run3, Run4)**: **957.93 tok/s**
...
**H100 Reference** (16K input, 1024 output): 4379.60 tok/s

**Achievement Calculation**:
```
Achievement = (A3_throughput / A3_compute) / (H100_throughput / H100_compute)
            = (957.93 / 6048) / (4379.60 / 15824)
            = 57.23%
```

**Target**: ≥80% (per Decision D-020)

**Status**: **BELOW TARGET** (57.23% < 80%)
...
- `16K/run1.log`, `16K/run2.log`, `16K/run3.log`, `16K/run4.log`: Benchmark output logs
- `16K/average_run2_4.txt`: Runner-computed aggregation
...
**User-Provided (XLSX, 2026-09-02)**: 957.94 tok/s  
**Evidence-Backed (Mean Run2-4)**: 957.93 tok/s  
...
- Workload contract verified (16384in/1024out, 256 prompts, ignore_eos=true)
...
**Rationale**: Evidence completeness, workload contract, runtime identity, and calculation integrity all verified. This Result establishes the formal 16K baseline for GLM-5.2-W8A8 on A3/910C hardware with the verified runtime (vLLM 0.6.4.post1+ascend).

**Performance Assessment**: Current throughput (957.93 tok/s) achieves 57.23% of the normalized target. This is **below the 80% threshold** per Decision D-020, establishing a performance gap for optimization work.
...
**Historical Note**: This is the first formal Evidence-backed Result for the 16K cell. User-measured value (957.94 tok/s, 2026-09-02) remains valid historical provenance.
