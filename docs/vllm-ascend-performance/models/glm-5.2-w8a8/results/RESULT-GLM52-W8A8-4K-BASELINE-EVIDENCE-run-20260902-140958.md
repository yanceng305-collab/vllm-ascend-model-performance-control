# Result: GLM-5.2-W8A8 4K Baseline (Evidence-Backed)

**Result ID**: `RESULT-GLM52-W8A8-4K-BASELINE-EVIDENCE-run-20260902-140958`  
**Model**: GLM-5.2-W8A8  
**Workload**: 4096 input tokens, 1024 output tokens  
...
- **Run 2**: 820.62 tok/s (completed: 256, failed: 0)
- **Run 3**: 820.49 tok/s (completed: 256, failed: 0)
- **Run 4**: 821.17 tok/s (completed: 256, failed: 0)

### Independent Calculation

**Mean (Run2, Run3, Run4)**: **820.76 tok/s**
...
**H100 Reference** (4K input, 1024 output): 4063.45 tok/s

**Achievement Calculation**:
```
Achievement = (A3_throughput / A3_compute) / (H100_throughput / H100_compute)
            = (820.76 / 6048) / (4063.45 / 15824)
            = 52.85%
```

**Target**: ≥80% (per Decision D-020)

**Status**: **BELOW TARGET** (52.85% < 80%)
...
- `4K/run1.log`, `4K/run2.log`, `4K/run3.log`, `4K/run4.log`: Benchmark output logs
- `4K/average_run2_4.txt`: Runner-computed aggregation
...
**User-Provided (XLSX, 2026-09-02)**: 820.76 tok/s  
**Evidence-Backed (Mean Run2-4)**: 820.76 tok/s  
**Difference**: 0.00 tok/s (0.00% variance)

**Assessment**: Values match exactly. Evidence-backed calculation confirms User-measured baseline.
...
- Workload contract verified (4096in/1024out, 256 prompts, ignore_eos=true)
...
- Matches User-provided value exactly
...
**Rationale**: Evidence completeness, workload contract, runtime identity, and calculation integrity all verified. This Result establishes the formal 4K baseline for GLM-5.2-W8A8 on A3/910C hardware with the verified runtime (vLLM 0.6.4.post1+ascend).

**Performance Assessment**: Current throughput (820.76 tok/s) achieves 52.85% of the normalized target. This is **below the 80% threshold** per Decision D-020, establishing a performance gap for optimization work.
...
**Historical Note**: This is the first formal Evidence-backed Result for the 4K cell. User-measured value (820.76 tok/s, 2026-09-02) remains valid historical provenance.
