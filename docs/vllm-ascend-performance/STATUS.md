...
**New exploratory Task (2026-09-02)**: `GLM52-W8A8-OFFICIAL-DERIVED-A3-64K-COMPAT-PROFILE-MICROGATE` created as **READY / PENDING USER DISPATCH** (Task + A3PerfRunner Prompt committed in the model dir). Official-derived upstream A3 GLM-5.2-W8A8 profile under the frozen `max-model-len=70000`: startup + 70K capacity gate, then 16K two-run microgate, no auto-64K; leave service running. Independent of formal OPT-01 (still BLOCKED_PENDING_BASELINE_VALUE_VERIFICATION); no server execution yet; no normalization/Decision change.
