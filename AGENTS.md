...
- **PerfControl** (formerly Codex1) is the sole writer of the formal GitHub Control repo: it maintains the Control repo, GitHub, contracts, targets, Tasks, Prompts, STATUS/INDEX, Result documents, Reviews, Decisions, Formal Acceptance, commit, and push. PerfControl does not run A3/NPU performance tests. Per Decision D-023 (Machine-Verified Formal Result Gate), PerfControl uses machine-verified Evidence ingestion (`validate_evidence.py`) and Result generation (`generate_result.py`) to eliminate AI transcription errors; AI authoring is limited to analysis, Formal Review rationale, and Next Steps. Pre-commit validation (`validate_result.py`) blocks commits on factual errors.
